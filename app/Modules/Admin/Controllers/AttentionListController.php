...
class AttentionListController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->display('attention_list.htm');
        }
        if ($_REQUEST['act'] == 'query') {
            ...
            $this->smarty->assign('record_count', $goodsdb['record_count']);
            $this->smarty->assign('page_count', $goodsdb['page_count']);
            make_json_result($this->smarty->fetch('attention_list.htm'), '',
        ...
        if ($_REQUEST['act'] == 'addtolist') {
            ...
            if ($count > $start) {
                ...
                foreach ($query as $rt) {
                    $time = time();
                    $goods_url = $this->ecs->url() . build_uri('goods', array('gid' => $id), $rt['goods_name']);
                    $this->smarty->assign(array('user_name' => $rt['user_name'], 'goods_name' => $rt['goods_name'], 'goods_url' => $goods_url, 'shop_name' => $GLOBALS['_CFG']['shop_title'], 'send_date' => local_date($GLOBALS['_CFG']['date_format'])));
        ...
        if ($_REQUEST['act'] == 'batch_addtolist') {
            ...
            if ($count > $start) {
                ...
                foreach ($query as $rt) {
                    ...
                    $goods_url = $this->ecs->url() . build_uri('goods', array('gid' => $rt['goods_id']), $rt['user_name']);

                    $this->smarty->assign(array('user_name' => $rt['user_name'], 'goods_name' => $rt['goods_name'], 'goods_url' => $goods_url));
                    $content = $this->smarty->fetch("str:$template[template_content]");
                    $add .= $add ? ",('$rt[email]','$template[template_id]','$content','$pri','$time')" : "('$rt[email]','$template[template_id]','$content','$pri','$time')";
                    $i++;
                }
