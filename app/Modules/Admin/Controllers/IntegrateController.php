...
//-- 会员数据整合插件列表
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->display('integrates_list.htm');
        }
...
//        }
                ...
                $this->smarty->display('integrates_setup.htm');
            }
        }
...
//-- 检查用户填写资料
        ...
        if ($_REQUEST['act'] == 'check_config') {
            ...
            $this->smarty->display('integrates_check.htm');
        }
...
//-- 第一次保存UCenter安装的资料
        ...
        if ($_REQUEST['act'] == 'save_uc_config_first') {
            ...
            $this->smarty->display('integrates_uc_import.htm');
        }
...
//-- 重名用户处理
        ...
        if ($_REQUEST['act'] == 'modify') {
            ...
            $this->smarty->display('integrates_modify.htm');
        }
...
//-- ajax 用户列表查询
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('integrates_modify.htm'), '', array('filter' => $arr['filter'], 'page_count' => $arr['page_count']));
        }
...
//-- 将商城数据同步到论坛
        ...
        if ($_REQUEST['act'] == 'sync') {
            ...
            session(['task' => array('del' => array('total' => $task_del, 'start' => 0), 'rename' => array('total' => $task_rename, 'start' => 0), 'sync' => array('total' => $task_sync, 'start' => 0))]);
            ...
            $this->smarty->assign('size', $size);
            $this->smarty->display('integrates_sync.htm');
        }
...
//-- 保存UCenter设置
        ...
        if ($_REQUEST['act'] == 'points_set') {
            ...
            $this->smarty->display('integrates_points.htm');
        }
