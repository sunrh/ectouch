...
class GoodsExportController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($_REQUEST['act'] == 'goods_export') {
            ...
            $this->smarty->assign('goods_type_list', goods_type_list(0));
            ...
            $this->smarty->display('goods_export.htm');
        }
        ...
        if ($_REQUEST['act'] == 'act_export_taobao') {
            ...
            if ($GLOBALS['_CFG']['shop_province'] || $GLOBALS['_CFG']['shop_city']) {
                $sql = "SELECT region_id,  region_name FROM " . $this->ecs->table('region') . " WHERE region_id IN ('{$GLOBALS['_CFG'][shop_province]}',  '{$GLOBALS['_CFG'][shop_city]}')";
        ...
        if ($arr) {
            ...
            $sql = "SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_number, g.goods_desc, g.goods_img " .
                " FROM " . $this->ecs->table('goods') . " AS g " . $where;
        ...
        if ($_REQUEST['act'] == 'import_taobao') {
            $this->smarty->display('import_taobao.htm');
            ...
            $sql = "SELECT g.*, b.brand_name as brandname " .
                " FROM " . $this->ecs->table('goods') . " AS g LEFT JOIN " . $this->ecs->table('brand') . " AS b " .
        ...
        if ($_REQUEST['act'] == 'import_paipai') {
            $this->smarty->display('import_paipai.htm');
        }
        /* 处理Ajax调用 */
        ...
        if ($_REQUEST['act'] == 'act_export_custom') {
            ...
            foreach ($res as $row) {
                ...
                $sql = "SELECT `attr_id`, `attr_value` FROM " . $this->ecs->table('goods_attr') . " WHERE `goods_id` = '" . $row['goods_id'] . "'";
