...
//-- 批量上传
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $this->smarty->display('goods_batch_add.htm');
        }
        ...
        if ($_REQUEST['act'] == 'upload') {
            ...
            if ($_POST['data_cat'] == 'ecshop') {
                ...
                foreach ($data AS $line) {
                    ...
                    $max_id = $this->db->getOne("SELECT MAX(goods_id) + $id_is FROM " . $this->ecs->table('goods'));
                    $id_is++;
            ...
            // 显示的字段列表
            $this->smarty->assign('field_show', array('goods_name' => true, 'goods_sn' => true, 'brand_name' => true, 'market_price' => true, 'shop_price' => true));
            ...
            $this->smarty->display('goods_batch_confirm.htm');
        }
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            if (isset($_POST['checked'])) {
                ...
                foreach ($_POST['checked'] AS $key => $value) {
                        ...
                        if (!empty($field_value)) {
                            ...
                            elseif ($field == 'brand_name') {
                                if (isset($brand_list[$field_value])) {
                                    ...
                                    $sql = "INSERT INTO " . $this->ecs->table('brand') . " (brand_name) VALUES ('" . addslashes($field_value) . "')";
                                    $this->db->query($sql);
                    ...
                    $this->db->autoExecute($this->ecs->table('goods'), $field_arr, 'INSERT');
                    ...
                    if (!empty($field_arr['original_img']) || !empty($field_arr['goods_img']) || !empty($field_arr['goods_thumb'])) {
                        ...
                        //修改商品图
                        $this->db->query("UPDATE " . $this->ecs->table('goods') . " SET goods_img = '$goods_img', goods_thumb = '$goods_thumb', original_img = '$original_img' WHERE goods_id='" . $goods_gallery['goods_id'] . "'");

                        //添加商品相册图
                        if ($GLOBALS['_CFG']['auto_generate_gallery']) {
                            $this->db->autoExecute($this->ecs->table('goods_gallery'), $goods_gallery, 'INSERT');
                        }
                    }
                }
            }
...
//-- 批量修改：选择商品
        ...
        if ($_REQUEST['act'] == 'select') {
            ...
            $this->smarty->display('goods_batch_select.htm');
        }
...
//-- 批量修改：修改
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            if ($_POST['select_method'] == 'cat') {
                ...
                $sql = "SELECT DISTINCT goods_id FROM " . $this->ecs->table('goods') .
                    " WHERE goods_sn " . db_create_in($goods_sns);
                $goods_ids = join(',', $this->db->getCol($sql));
            ...
            if (!empty($product_list)) {
                ...
                $this->smarty->assign('product_list', $_product_list);
            ...
            $this->smarty->display('goods_batch_edit.htm');
        }
                    ...
                    foreach ($_POST['goods_id'] AS $goods_id) {
                        ...
                        if (!empty($_POST['product_number'][$goods_id])) {
                            ...
                            foreach ($_POST['product_number'][$goods_id] as $key => $value) {
                                $this->db->autoExecute($this->ecs->table('products'), array('product_number', $value), 'UPDATE', "goods_id = '$goods_id' AND product_id = " . $key);
                        ...
                        $this->db->autoExecute($this->ecs->table('goods'), $goods, 'UPDATE', "goods_id = '$goods_id'");
                        ...
                        if (!empty($_POST['rank_id'])) {
                            foreach ($_POST['rank_id'] AS $rank_id) {
                                ...
                                $sql = "SELECT COUNT(*) FROM " . $this->ecs->table('member_price') . " WHERE goods_id = '$goods_id' AND user_rank = '$rank_id'";
                                if ($this->db->getOne($sql) > 0) {
                                    if ($rank['user_price'] < 0) {
                                        $this->db->query("DELETE FROM " . $this->ecs->table('member_price') . " WHERE goods_id = '$goods_id' AND user_rank = '$rank_id'");
                                    } else {
