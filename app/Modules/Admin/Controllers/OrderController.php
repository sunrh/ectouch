...
//-- 订单查询
        ...
        if ($_REQUEST['act'] == 'order_query') {
            ...
            /* 模板赋值 */
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['03_order_query']);
            $this->smarty->assign('action_link', array('href' => 'order.php?act=list', 'text' => $GLOBALS['_LANG']['02_order_list']));

            /* 显示模板 */

            $this->smarty->display('order_query.htm');
        }
...
//-- 排序、分页、查询
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('order_list.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
        }
...
//-- 订单详情页面
        ...
        if ($_REQUEST['act'] == 'info') {
            ...
            $sql = "SELECT agency_id FROM " . $this->ecs->table('admin_user') . " WHERE user_id = '" . session('admin_id') . "'";
            ...
            /* 取得上一个、下一个订单号 */
            if (!empty(request()->cookie('ectouch_cp_lastfilter'))) {
            ...
            $sql = "SELECT MAX(order_id) FROM " . $this->ecs->table('order_info') . " as o WHERE order_id < '$order[order_id]'";
            if ($agency_id > 0) {
            ...
            $sql = "SELECT MIN(order_id) FROM " . $this->ecs->table('order_info') . " as o WHERE order_id > '$order[order_id]'";
            if ($agency_id > 0) {
                $sql .= " AND agency_id = '$agency_id'";
            }
            if (!empty($where)) {
            ...
            /* 取得所有办事处 */
            $sql = "SELECT agency_id, agency_name FROM " . $this->ecs->table('agency');
            ...
            $sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
                ...
                "LEFT JOIN " . $this->ecs->table('region') . " AS c ON o.country = c.region_id " .
                "LEFT JOIN " . $this->ecs->table('region') . " AS p ON o.province = p.region_id " .
                "LEFT JOIN " . $this->ecs->table('region') . " AS t ON o.city = t.region_id " .
                "LEFT JOIN " . $this->ecs->table('region') . " AS d ON o.district = d.region_id " .
            ...
            if ($order['from_ad'] == 0) {
                ...
                /* 查询广告的名称 */
                $ad_name = $this->db->getOne("SELECT ad_name FROM " . $this->ecs->table('ad') . " WHERE ad_id='$order[from_ad]'");
                $order['referer'] = $GLOBALS['_LANG']['from_ad_js'] . $ad_name . ' (' . $GLOBALS['_LANG']['from'] . $order['referer'] . ')';
            }

            /* 此订单的发货备注(此订单的最后一条操作记录) */
            $sql = "SELECT action_note FROM " . $this->ecs->table('order_action') .
            ...
            if ($order['user_id'] > 0) {
                ...
                $sql = "SELECT rank_name FROM " . $this->ecs->table('user_rank') . $where;
                ...
                $sql = "SELECT COUNT(*) " .
                    "FROM " . $this->ecs->table('bonus_type') . " AS bt, " . $this->ecs->table('user_bonus') . " AS ub " .
                    "WHERE bt.type_id = ub.bonus_type_id " .
                    "AND ub.user_id = '$order[user_id]' " .
                    "AND ub.order_id = 0 " .
                    "AND bt.use_start_date <= '$today' " .
                    "AND bt.use_end_date >= '$today'";
                $user['bonus_count'] = $this->db->getOne($sql);
                $this->smarty->assign('user', $user);

                // 地址信息
                $sql = "SELECT * FROM " . $this->ecs->table('user_address') . " WHERE user_id = '$order[user_id]'";
            ...
            FROM " . $this->ecs->table('order_goods') . " AS o
                LEFT JOIN " . $this->ecs->table('products') . " AS p
                    ON p.product_id = o.product_id
                LEFT JOIN " . $this->ecs->table('goods') . " AS g
                    ON o.goods_id = g.goods_id
                LEFT JOIN " . $this->ecs->table('brand') . " AS b
            ...
            /* 取得订单操作记录 */
            $act_list = array();
            $sql = "SELECT * FROM " . $this->ecs->table('order_action') . " WHERE order_id = '$order[order_id]' ORDER BY log_time DESC,action_id DESC";
            $res = $this->db->query($sql);
            foreach ($res as $row) {
            ...
            /* 取得是否存在实体商品 */
            $this->smarty->assign('exist_real_goods', exist_real_goods($order['order_id']));
            ...
            if (isset($_GET['print'])) {
                ...
                $this->smarty->display('order_print.html');
            } /* 打印快递单 */
            elseif (isset($_GET['shipping_print'])) {
                ...
                $region = $this->db->getAll("SELECT region_id, region_name FROM " . $this->ecs->table("region") . " WHERE region_id IN ($region_id)");
                if (!empty($region)) {
                ...
                $this->smarty->assign('province', $region_array[$GLOBALS['_CFG']['shop_province']]);
                $this->smarty->assign('city', $region_array[$GLOBALS['_CFG']['shop_city']]);
                $this->smarty->assign('shop_address', $GLOBALS['_CFG']['shop_address']);
                $this->smarty->assign('service_phone', $GLOBALS['_CFG']['service_phone']);
                $shipping = $this->db->getRow("SELECT * FROM " . $this->ecs->table("shipping") . " WHERE shipping_id = " . $order['shipping_id']);
                ...
                if ($shipping['print_model'] == 2) {
                    ...
                    $this->smarty->display('print.htm');
                } elseif (!empty($shipping['shipping_print'])) {
                    /* 代码 */
                    echo $this->smarty->fetch("str:" . $shipping['shipping_print']);
                } else {
                    $shipping_code = $this->db->getOne("SELECT shipping_code FROM " . $this->ecs->table('shipping') . " WHERE shipping_id=" . $order['shipping_id']);
                    if ($shipping_code) {
                ...
                $this->smarty->display('order_info.htm');
            }
        }
...
//-- 发货单列表
        ...
        if ($_REQUEST['act'] == 'delivery_list') {
            ...
            $this->smarty->display('delivery_list.htm');
        }
...
//-- 搜索、排序、分页
        ...
        if ($_REQUEST['act'] == 'delivery_query') {
            ...
            $this->smarty->assign($sort_flag['tag'], $sort_flag['img']);
            make_json_result($this->smarty->fetch('delivery_list.htm'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
        }
...
//-- 发货单详细
        ...
        if ($_REQUEST['act'] == 'delivery_info') {
            ...
            if ($agency_id > 0) {
                ...
                /* 取当前办事处信息 */
                $sql = "SELECT agency_name FROM " . $this->ecs->table('agency') . " WHERE agency_id = '$agency_id' LIMIT 0, 1";
            ...
            /* 取得发货单商品 */
            $goods_sql = "SELECT *
                  FROM " . $this->ecs->table('delivery_goods') . "
            ...
            /* 取得订单操作记录 */
            $act_list = array();
            $sql = "SELECT * FROM " . $this->ecs->table('order_action') . " WHERE order_id = '" . $delivery_order['order_id'] . "' AND action_place = 1 ORDER BY log_time DESC,action_id DESC";
            $res = $this->db->query($sql);
            foreach ($res as $row) {
                $row['order_status'] = $GLOBALS['_LANG']['os'][$row['order_status']];
                $row['pay_status'] = $GLOBALS['_LANG']['ps'][$row['pay_status']];
            ...
            $this->smarty->display('delivery_info.htm');
            exit; //
        }
...
//-- 发货单发货确认
        ...
        GROUP BY DG.goods_id ";
            ...
            $query = $this->db->autoExecute($this->ecs->table('delivery_order'), $_delivery, 'UPDATE', "delivery_id = $delivery_id", 'SILENT');
            if (!$query) {
                /* 操作失败 */
            ...
            if ($order_finish) {
                ...
                if ($cfg == '1') {
                    ...
                    $this->smarty->assign('confirm_url', $this->ecs->url() . 'receive.php?id=' . $order['order_id'] . '&con=' . rawurlencode($order['consignee']));
                    $this->smarty->assign('send_msg_url', $this->ecs->url() . 'user.php?act=message_list&order_id=' . $order['order_id']);
...
//-- 退货单列表
        ...
        if ($_REQUEST['act'] == 'back_list') {
            ...
            $this->smarty->display('back_list.htm');
        }
...
//-- 搜索、排序、分页
        ...
        if ($_REQUEST['act'] == 'back_query') {
            ...
            $sort_flag = sort_flag($result['filter']);
            $this->smarty->assign($sort_flag['tag'], $sort_flag['img']);
            make_json_result($this->smarty->fetch('back_list.htm'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
        }
...
//-- 退货单详细
        ...
        if ($_REQUEST['act'] == 'back_info') {
            ...
            $back_order['region'] = $this->db->getOne($sql);
            ...
            /* 取得发货单商品 */
            $goods_sql = "SELECT *
                  FROM " . $this->ecs->table('back_goods') . "
            ...
            $this->smarty->assign('action_link', array('href' => 'order.php?act=back_list&' . list_link_postfix(), 'text' => $GLOBALS['_LANG']['10_back_order']));

            $this->smarty->display('back_info.htm');
            exit; //
        }
        ...
        if ($_REQUEST['act'] == 'step_post') {
            ...
            if ('user' == $step) {
                ...
                do {
                    ...
                    if ($this->db->autoExecute($this->ecs->table('order_info'), $order, 'INSERT', '', 'SILENT')) {
                        break;
                    } else {
            ...
            elseif ('add_goods' == $step) {
                ...
                if ($GLOBALS['_CFG']['use_storage'] == '1' && $GLOBALS['_CFG']['stock_dec_time'] == SDT_PLACE) {
                    ...
                    if (!empty($product_info['product_id'])) {
                        $sql = "UPDATE " . $this->ecs->table('products') . "
                    ...
                    $sql = "UPDATE " . $this->ecs->table('goods') .
                ...
                if ($old_order['user_id'] > 0) {
                    ...
                    if ($old_order['bonus_id'] != $order['bonus_id']) {
                        if ($old_order['bonus_id'] > 0) {
                            $sql = "UPDATE " . $this->ecs->table('user_bonus') .
...
//-- 修改订单（载入页面）
        ...
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            /* 取得参数 act */
            $act = $_GET['act'];
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['add_order']);
            ...
            elseif ('money' == $step) {
                ...
                if ($order['user_id'] > 0) {
                    ...
                    /* 计算可用余额 */
                    $this->smarty->assign('available_user_money', $order['surplus'] + $user['user_money']);

                    /* 计算可用积分 */
                    $this->smarty->assign('available_pay_points', $order['integral'] + $user['pay_points']);
...
//        }
            ...
            $this->smarty->display('order_step.htm');
        }
...
//-- 处理
        ...
        if ($_REQUEST['act'] == 'process') {
            ...
            if ('drop_order_goods' == $func) {
                ...
                /* 如果使用库存，且下订单时减库存，则修改库存 */
                if ($GLOBALS['_CFG']['use_storage'] == '1' && $GLOBALS['_CFG']['stock_dec_time'] == SDT_PLACE) {
                    $goods = $this->db->getRow("SELECT goods_id, goods_number FROM " . $this->ecs->table('order_goods') . " WHERE rec_id = " . $rec_id);
                    $sql = "UPDATE " . $this->ecs->table('goods') .
                ...
                /* 删除 */
                $sql = "DELETE FROM " . $this->ecs->table('order_goods') .
                    " WHERE rec_id = '$rec_id' LIMIT 1";
                $this->db->query($sql);
            ...
            elseif ('cancel_order' == $func) {
                ...
                if ($step_act == 'add') {
                    /* 如果是添加，删除订单，返回订单列表 */
                    if ($order_id > 0) {
                        $sql = "DELETE FROM " . $this->ecs->table('order_info') .
            ...
            elseif ('load_refund' == $func) {
                ...
                /* 显示模板 */
                $this->smarty->assign('ur_here', $GLOBALS['_LANG']['refund']);

                $this->smarty->display('order_refund.htm');
            } else {
                die('invalid params');
            }
        }
...
//-- 合并订单
        ...
        if ($_REQUEST['act'] == 'merge') {
            ...
            $sql = "SELECT o.order_sn, u.user_name " .
                "FROM " . $this->ecs->table('order_info') . " AS o " .
                "LEFT JOIN " . $this->ecs->table('users') . " AS u ON o.user_id = u.user_id " .
                "WHERE o.user_id > 0 " .
                "AND o.extension_code = '' " . order_query_sql('unprocessed');
            $this->smarty->assign('order_list', $this->db->getAll($sql));

            /* 模板赋值 */
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['04_merge_order']);
            $this->smarty->assign('action_link', array('href' => 'order.php?act=list', 'text' => $GLOBALS['_LANG']['02_order_list']));

            /* 显示模板 */

            $this->smarty->display('merge_order.htm');
        }
...
//-- 订单打印模板（载入页面）
        ...
        if ($_REQUEST['act'] == 'templates') {
            ...
            /* 模板赋值 */
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['edit_order_templates']);
            $this->smarty->assign('action_link', array('href' => 'order.php?act=list', 'text' => $GLOBALS['_LANG']['02_order_list']));
            $this->smarty->assign('act', 'edit_templates');

            /* 显示模板 */

            $this->smarty->display('order_templates.htm');
        }
        /*------------------------------------------------------ */
//-- 订单打印模板（提交修改）
        /*------------------------------------------------------ */
...
//-- 操作订单状态（载入页面）
        ...
        if ($_REQUEST['act'] == 'operate') {
            ...
            elseif (isset($_POST['ship'])) {
                ...
                $this->smarty->display('order_delivery_info.htm');
                exit;
            } /* 未发货 */
            ...
            elseif (isset($_POST['remove'])) {
                ...
                if (!$batch) {
                    ...
                    /* 删除订单 */
                    $this->db->query("DELETE FROM " . $this->ecs->table('order_info') . " WHERE order_id = '$order_id'");
                    $this->db->query("DELETE FROM " . $this->ecs->table('order_goods') . " WHERE order_id = '$order_id'");
                    $this->db->query("DELETE FROM " . $this->ecs->table('order_action') . " WHERE order_id = '$order_id'");
            ...
            elseif (isset($_REQUEST['remove_invoice'])) {
                ...
                foreach ($delivery_id as $value_is) {
                    ...
                    // 更新：删除发货单
                    $sql = "DELETE FROM " . $this->ecs->table('delivery_order') . " WHERE delivery_id = '$value_is'";
                    $this->db->query($sql);
                }
            ...
            elseif (isset($_REQUEST['remove_back'])) {
                ...
                if (is_array($back_id)) {
                    foreach ($back_id as $value_is) {
                        $sql = "DELETE FROM " . $this->ecs->table('back_order') . " WHERE back_id = '$value_is'";
                        $this->db->query($sql);
                    }
                } else {
                    $sql = "DELETE FROM " . $this->ecs->table('back_order') . " WHERE back_id = '$back_id'";
                    $this->db->query($sql);
                }
                /* 返回 */
            ...
            elseif (isset($_POST['print'])) {
                ...
                foreach ($order_sn_list as $order_sn) {
                    ...
                    $sql = "SELECT o.*, g.goods_number AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name " .
                        "FROM " . $this->ecs->table('order_goods') . " AS o " .
                        "LEFT JOIN " . $this->ecs->table('goods') . " AS g ON o.goods_id = g.goods_id " .
            ...
            if (($require_note && $action_note == '') || isset($show_invoice_no) || isset($show_refund)) {
                ...
                $this->smarty->display('order_operate.htm');
            } else {
                /* 直接处理 */
                if (!$batch) {
                    /* 一个订单 */
        ...
        if ($_REQUEST['act'] == 'batch_operate_post') {
            ...
            if ('confirm' == $operation) {
                foreach ($order_id_list as $id_order) {
                    $sql = "SELECT * FROM " . $this->ecs->table('order_info') .
            ...
            elseif ('invalid' == $operation) {
                ...
                foreach ($order_id_list as $id_order) {
                    ...
                    /* 删除订单 */
                    $this->db->query("DELETE FROM " . $this->ecs->table('order_info') . " WHERE order_id = '$order[order_id]'");
                    $this->db->query("DELETE FROM " . $this->ecs->table('order_goods') . " WHERE order_id = '$order[order_id]'");
                    $this->db->query("DELETE FROM " . $this->ecs->table('order_action') . " WHERE order_id = '$order[order_id]'");
                    $action_array = array('delivery', 'back');
...
//    $action_note = $_REQUEST['action_note'];
            ...
            if (empty($sn_not_list)) {
                ...
                $this->smarty->display('order_operate_info.htm');
            }
        }
        ...
        if ($_REQUEST['act'] == 'operate_post') {
            ...
            elseif ('split' == $operation) {
                ...
                if ($order['order_status'] == OS_SPLITED) {
                    ...
                    sys_msg(sprintf($GLOBALS['_LANG']['order_splited_sms'], $order['order_sn'],
                ...
                /* 发货单入库 */
                $query = $this->db->autoExecute($this->ecs->table('delivery_order'), $_delivery, 'INSERT', '', 'SILENT');
                    ...
                    if (!empty($goods_list)) {
                        foreach ($goods_list as $value) {
                            ...
                            if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card') {
                                ...
                                $query = $this->db->autoExecute($this->ecs->table('delivery_goods'), $delivery_goods, 'INSERT', '', 'SILENT');
                            } // 商品（超值礼包）
                            elseif ($value['extension_code'] == 'package_buy') {
                                foreach ($value['package_goods_list'] as $pg_key => $pg_value) {
                                    ...
                                    $query = $this->db->autoExecute($this->ecs->table('delivery_goods'), $delivery_pg_goods, 'INSERT', '', 'SILENT');
                                }
                            }
                        }
                    }
            ...
            elseif ('return' == $operation) {
                ...
                WHERE order_id = '" . $order['order_id'] . "'";
                    ...
                    $goods_num = $this->db->query($sql);
                    $goods_num = $this->db->fetchRow($goods_num);
                ...
                if ($delivery_list) {
                    foreach ($delivery_list as $list) {
                        ...
                        $sql_back_goods = "INSERT INTO " . $this->ecs->table('back_goods') . " (back_id, goods_id, product_id, product_sn, goods_name,goods_sn, is_real, send_number, goods_attr)
                ...
                /* 修改订单的发货单状态为退货 */
                $sql_delivery = "UPDATE " . $this->ecs->table('delivery_order') . "
                         SET status = 1
                         WHERE status IN (0, 2)
                         AND order_id = " . $order['order_id'];
        ...
        if ($_REQUEST['act'] == 'json') {
            ...
            if ($func == 'get_goods_info') {
                ...
                $sql = "SELECT goods_id, c.cat_name, goods_sn, goods_name, b.brand_name, " .
                    ...
                    "LEFT JOIN " . $this->ecs->table('brand') . " AS b ON g.brand_id = b.brand_id " .
                    "LEFT JOIN " . $this->ecs->table('category') . " AS c ON g.cat_id = c.cat_id " .
                ...
                $sql = "SELECT p.user_price, r.rank_name " .
                    "FROM " . $this->ecs->table('member_price') . " AS p, " .
                    $this->ecs->table('user_rank') . " AS r " .
...
//-- 获取订单商品信息
        ...
        if ($_REQUEST['act'] == 'get_goods_info') {
            ...
            foreach ($res as $row) {
                ...
                $_goods_thumb = (strpos($_goods_thumb, 'http://') === 0) ? $_goods_thumb : $this->ecs->url() . $_goods_thumb;
    ...
    private function order_list()
    {
        ...
        if ($result === false) {
            ...
            $sql = "SELECT agency_id FROM " . $GLOBALS['ecs']->table('admin_user') . " WHERE user_id = '" . session('admin_id') . "'";
