 ...
 */
class PictureBatchController extends Controller
{
    private function actionIndex()
    {
        ...
        if (empty($_GET['is_ajax'])) {
            ...
            $this->smarty->assign('cat_list', cat_list(0, 0));
            $this->smarty->assign('brand_list', get_brand_list());
            $this->smarty->display('picture_batch.htm');
            ...
            $sql = 'SELECT `goods_id`, `goods_name` FROM ' . $this->ecs->table('goods') . ' AS g WHERE 1 ' . $goods_where . ' LIMIT 50';
            ...
            if (!empty($goods_where)) {
                $album_where = ', ' . $this->ecs->table('goods') . " AS g WHERE album.img_original > '' AND album.goods_id = g.goods_id " . $goods_where;
            } else {
            ...
            if (isset($_GET['start'])) {
                ...
                if (isset($_GET['total_icon'])) {
                    $count = $this->db->GetOne("SELECT COUNT(*) FROM " . $this->ecs->table('goods') . " AS g WHERE g.original_img <> ''" . $goods_where);
