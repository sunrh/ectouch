...
//-- 活动列表页
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['wholesale_list']);
            ...
            $this->smarty->assign('wholesale_list', $list['item']);
            ...
            $this->smarty->display('wholesale_list.htm');
        }
...
//-- 分页、排序、查询
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('wholesale_list.htm'), '',
...
//-- 删除
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            /* 删除记录 */
            $sql = "DELETE FROM " . $this->ecs->table('wholesale') .
...
//-- 修改排序
        ...
        if ($_REQUEST['act'] == 'toggle_enabled') {
            ...
            $sql = "UPDATE " . $this->ecs->table('wholesale') .
                " SET enabled = '$val'" .
                " WHERE act_id = '$id' LIMIT 1";
            $this->db->query($sql);
...
//-- 批量添加
        ...
        if ($_REQUEST['act'] == 'batch_add') {
            ...
            $this->smarty->display('wholesale_batch_info.htm');
        }
...
//-- 批量添加入库
        ...
        if ($_REQUEST['act'] == 'batch_add_insert') {
            ...
            $sql = "SELECT goods_name, goods_id FROM " . $this->ecs->table('goods') .
            ...
            if (isset($_POST['rank_id'])) {
                ...
                foreach ($_POST['rank_id'] as $rank_id) {
                    $sql = "SELECT COUNT(act_id) AS num, goods_id FROM " . $this->ecs->table('wholesale') .
                        " WHERE goods_id IN ($dst_goods) " .
            ...
            foreach ($_POST['dst_goods_lists'] as $goods_value) {
                ...
                /* 保存数据 */
                $this->db->autoExecute($this->ecs->table('wholesale'), $_wholesale, 'INSERT');
...
//-- 添加、编辑
        ...
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            if ($is_add) {
                ...
                /* 取得商品属性 */
                $this->smarty->assign('attr_list', get_goods_attr($wholesale['goods_id']));
            }
            ...
            $this->smarty->display('wholesale_info.htm');
        }
...
//-- 添加、编辑后提交
        ...
        if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if (isset($_POST['rank_id'])) {
                foreach ($_POST['rank_id'] as $rank_id) {
                    $sql = "SELECT COUNT(*) FROM " . $this->ecs->table('wholesale') .
                        " WHERE goods_id = '$goods_id' " .
                        " AND CONCAT(',', rank_ids, ',') LIKE CONCAT('%,', '$rank_id', ',%')";
                    if (!$is_add) {
            ...
            $sql = "SELECT a.attr_id " .
                "FROM " . $this->ecs->table('goods') . " AS g, " . $this->ecs->table('attribute') . " AS a " .
                "WHERE g.goods_id = '$goods_id' " .
                "AND g.goods_type = a.cat_id " .
                "AND a.attr_type = 1";
            $attr_id_list = $this->db->getCol($sql);
            ...
            foreach ($key_list as $key) {
                ...
                if (!empty($attr)) {
                    ...
                    $sql = "SELECT product_id FROM " . $this->ecs->table('products') . " WHERE goods_attr = '$goods_attr' AND goods_id = '$goods_id'";
            ...
            /* 保存数据 */
            if ($is_add) {
                $this->db->autoExecute($this->ecs->table('wholesale'), $wholesale, 'INSERT');
                $wholesale['act_id'] = $this->db->insert_id();
            } else {
                $this->db->autoExecute($this->ecs->table('wholesale'), $wholesale, 'UPDATE', "act_id = '$wholesale[act_id]'");
            }
