...
class DatabaseController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($_REQUEST['act'] == 'backup') {
            $tables = $this->db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($this->ecs->prefix) . "%'");
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['restore'], 'href' => 'database.php?act=restore'));
            $this->smarty->assign('tables', $tables);
            $this->smarty->assign('vol_size', $allow_max_size);
            $this->smarty->assign('sql_name', cls_sql_dump::get_random_name() . '.sql');
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['02_db_manage']);
            $this->smarty->display('db_backup.htm');
        }
        ...
        if ($_REQUEST['act'] == 'restore') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['02_db_manage'], 'href' => 'database.php?act=backup'));
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['restore']);
            $this->smarty->assign('list', $list);
            $this->smarty->display('db_restore.htm');
        }
        ...
        if ($_REQUEST['act'] == 'dumpsql') {
            ...
            switch ($type) {
                case 'full':
                    $except = array($this->ecs->prefix . 'sessions', $this->ecs->prefix . 'sessions_data');
                    $temp = $this->db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($this->ecs->prefix) . "%'");
                ...
                case 'stand':
                    ...
                    foreach ($temp as $table) {
                        $tables[$this->ecs->prefix . $table] = -1;
                    }
                    $dump->put_tables_list($run_log, $tables);
                    break;
            ...
            if (empty($tables)) {
                ...
                if ($vol > 1) {
                    ...
                    $this->smarty->assign('list', $list);
                    $this->smarty->assign('title', $GLOBALS['_LANG']['backup_success']);
                    $this->smarty->display('sql_dump_msg.htm');
                } else {
                    /* 只有一个文件 */
                    ...
                    $this->smarty->assign('list', array(array('name' => $sql_file_name . '.sql', 'href' => '../' . DATA_DIR . '/sqldata/' . $sql_file_name . '.sql')));
                    $this->smarty->assign('title', $GLOBALS['_LANG']['backup_success']);
                    $this->smarty->display('sql_dump_msg.htm');
                }
            } else {
                ...
                $this->smarty->assign('title', sprintf($GLOBALS['_LANG']['backup_title'], '#' . $vol));
                $this->smarty->assign('auto_redirect', 1);
                $this->smarty->assign('auto_link', $lnk);
                $this->smarty->display('sql_dump_msg.htm');
            }
        }
...
//-- 优化页面
        ...
        if ($_REQUEST['act'] == 'optimize') {
            /* 初始化数据 */
            admin_priv('db_backup');
            $db_ver_arr = $this->db->version();
            $db_ver = $db_ver_arr;
            $ret = $this->db->query("SHOW TABLE STATUS LIKE '" . mysql_like_quote($this->ecs->prefix) . "%'");
            ...
            foreach ($ret as $row) {
                if (strpos($row['Name'], '_session') !== false) {
                    ...
                    $res = $this->db->GetRow('CHECK TABLE ' . $row['Name']);
                    $num += $row['Data_free'];
                }
            ...
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['03_db_optimize']);
            $this->smarty->display('optimize.htm');
        }
        ...
        if ($_REQUEST['act'] == 'run_optimize') {
            ...
            foreach ($tables as $table) {
                if ($row = $this->db->getRow('OPTIMIZE TABLE ' . $table)) {
                    /* 优化出错，尝试修复 */
