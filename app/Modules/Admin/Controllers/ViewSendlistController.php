...
class ViewSendlistController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->display('view_sendlist.htm');
        }
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('view_sendlist.htm'), '', array('filter' => $listdb['filter'], 'page_count' => $listdb['page_count']));
        }
        if ($_REQUEST['act'] == 'del') {
            $id = (int)$_REQUEST['id'];
...
//-- 批量发送
        ...
        if ($_REQUEST['act'] == 'batch_send') {
            ...
            if (isset($_POST['checkboxes'])) {
                ...
                $sql = "SELECT * FROM " . $this->ecs->table('email_sendlist') . "WHERE id " . db_create_in($_POST['checkboxes']) . " ORDER BY pri DESC, last_send ASC";
...
//-- 全部发送
        ...
        if ($_REQUEST['act'] == 'all_send') {
            ...
            $sql = "SELECT * FROM " . $this->ecs->table('email_sendlist') . " ORDER BY pri DESC, last_send ASC";
            $res = $this->db->query($sql);
            foreach ($res as $row) {
                //发送列表不为空，邮件地址为空
                if (!empty($row['id']) && empty($row['email'])) {
