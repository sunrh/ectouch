...
//-- 补货处理
        ...
        if ($_REQUEST['act'] == 'replenish') {
            ...
            if (empty($_REQUEST['goods_id'])) {
                ...
                $goods_name = $this->db->GetOne("SELECT goods_name From " . $this->ecs->table('goods') . " WHERE goods_id='" . $_REQUEST['goods_id'] . "' AND is_real = 0 AND extension_code='virtual_card' ");
                if (empty($goods_name)) {
                    $link[] = array('text' => $GLOBALS['_LANG']['go_back'], 'href' => 'virtual_card.php?act=list');
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['go_list'], 'href' => 'virtual_card.php?act=card&goods_id=' . $card['goods_id']));
            $this->smarty->display('replenish_info.htm');
        }
...
//-- 补货列表
        ...
        if ($_REQUEST['act'] == 'card') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['replenish'],
            ...
            $this->smarty->display('replenish_list.htm');
        }
...
//-- 虚拟卡列表，用于排序、翻页
        ...
        if ($_REQUEST['act'] == 'query_card') {
            ...
            make_json_result($this->smarty->fetch('replenish_list.htm'), '',
        ...
        if ($_REQUEST['act'] == 'batch_card_add') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['virtual_card_list'], 'href' => 'goods.php?act=list&extension_code=virtual_card'));
            $this->smarty->assign('goods_id', $_REQUEST['goods_id']);
            $this->smarty->display('batch_card_info.htm');
        }
        if ($_REQUEST['act'] == 'batch_confirm') {
            ...
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['batch_card_add']);
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['batch_card_add'], 'href' => 'virtual_card.php?act=batch_card_add&goods_id=' . $_REQUEST['goods_id']));
            $this->smarty->assign('list', $rec);
            $this->smarty->display('batch_card_confirm.htm');
        }
        /* 批量上传处理 */
        if ($_REQUEST['act'] == 'batch_insert') {
            ...
            foreach ($_POST['checked'] as $key) {
                ...
                $this->db->AutoExecute($this->ecs->table('virtual_card'), $rec, 'INSERT');
                $i++;
            }
...
//-- 更改加密串
        ...
        if ($_REQUEST['act'] == 'change') {
            ...
            $this->smarty->display('virtual_card_change.htm');
        }
...
//-- 提交更改
        ...
        if ($_REQUEST['act'] == 'submit_change') {
            ...
            if (isset($_POST['old_string']) && isset($_POST['new_string'])) {
                ...
                $sql = "SELECT card_id, card_sn, card_password FROM " . $this->ecs->table('virtual_card') . " WHERE crc32 = '$old_crc32'";
                ...
                foreach ($res as $row) {
                    ...
                    $row['crc32'] = $new_crc32;
                    $this->db->autoExecute($this->ecs->table('virtual_card'), $row, 'UPDATE', 'card_id = ' . $row['card_id']);
                }
...
//-- 删除卡片
        ...
        if ($_REQUEST['act'] == 'remove_card') {
            ...
            $row = $this->db->GetRow('SELECT card_sn, goods_id FROM ' . $this->ecs->table('virtual_card') . " WHERE card_id = '$id'");
...
//-- 开始更改加密串：先检查原串和新串
        ...
        if ($_REQUEST['act'] == 'start_change') {
            ...
            $sql = "SELECT crc32, count(*) AS cnt FROM " . $this->ecs->table('virtual_card') . " GROUP BY crc32";
...
//-- 更新加密串
        ...
        if ($_REQUEST['act'] == 'on_change') {
            ...
            $sql = "SELECT card_id, card_sn, card_password " .
                " FROM " . $this->ecs->table('virtual_card') .
            ...
            foreach ($res as $row) {
                ...
                if (!$this->db->autoExecute($this->ecs->table('virtual_card'), $row, 'UPDATE', 'card_id = ' . $row['card_id'])) {
            ...
            // 查询是否还有未更新的
            $sql = "SELECT COUNT(*) FROM " . $this->ecs->table('virtual_card') . " WHERE crc32 = '$old_crc32' ";
