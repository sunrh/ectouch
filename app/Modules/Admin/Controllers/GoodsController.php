...
class GoodsController extends Controller
{
    public function actionIndex()
    {
        ...
        $exc = new Exchange($this->ecs->table('goods'), $this->db, 'goods_id', 'goods_name');
        ...
        if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash') {
            ...
            $this->smarty->assign('intro_list', get_intro_list());
        ...
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
            ...
            if ($is_add) {
                ...
                if ($_REQUEST['act'] == 'copy') {
                    ...
                    $sql = "SELECT '0' AS goods_id, link_goods_id, is_double, '" . session('admin_id') . "' AS admin_id" .
                        " FROM " . $this->ecs->table('link_goods') .
                        " WHERE goods_id = '$_REQUEST[goods_id]' ";
                    $res = $this->db->query($sql);
                    foreach ($res as $row) {
                        $this->db->autoExecute($this->ecs->table('link_goods'), $row, 'INSERT');
                    }

                    $sql = "SELECT goods_id, '0' AS link_goods_id, is_double, '" . session('admin_id') . "' AS admin_id" .
                        " FROM " . $this->ecs->table('link_goods') .
                    ...
                    $sql = "SELECT 0 AS parent_id, goods_id, goods_price, '" . session('admin_id') . "' AS admin_id " .
                        "FROM " . $this->ecs->table('group_goods') .
                        " WHERE parent_id = '$_REQUEST[goods_id]' ";
                    $res = $this->db->query($sql);
                    foreach ($res as $row) {
                        $this->db->autoExecute($this->ecs->table('group_goods'), $row, 'INSERT');
                    }
                    ...
                    $sql = "SELECT 0 AS goods_id, article_id, '" . session('admin_id') . "' AS admin_id " .
                        "FROM " . $this->ecs->table('goods_article') .
                        " WHERE goods_id = '$_REQUEST[goods_id]' ";
                    $res = $this->db->query($sql);
                    foreach ($res as $row) {
                        $this->db->autoExecute($this->ecs->table('goods_article'), $row, 'INSERT');
                    }
                    ...
                    $res = $this->db->query($sql);
                    foreach ($res as $row) {
                        $this->db->autoExecute($this->ecs->table('goods_attr'), addslashes_deep($row), 'INSERT');
                    }
                }

                // 扩展分类
                $other_cat_list = array();
                $sql = "SELECT cat_id FROM " . $this->ecs->table('goods_cat') . " WHERE goods_id = '$_REQUEST[goods_id]'";
                $goods['other_cat'] = $this->db->getCol($sql);
                foreach ($goods['other_cat'] as $cat_id) {
                    $other_cat_list[$cat_id] = cat_list(0, $cat_id);
                }
                $this->smarty->assign('other_cat_list', $other_cat_list);
            ...
            $this->smarty->assign('cat_list', cat_list(0, $goods['cat_id']));
            $this->smarty->assign('brand_list', get_brand_list());
            $this->smarty->assign('unit_list', get_unit_list());
            ...
            if (!$is_add) {
                $this->smarty->assign('member_price_list', get_member_price_list($_REQUEST['goods_id']));
            }
            ...
            $this->smarty->assign('goods_type_list', goods_type_list($goods['goods_type']));
            ...
            $this->smarty->display('goods_info.htm');
        }
...
//-- 插入商品 更新商品
        ...
        if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($is_insert) {
                ...
                $sql = "UPDATE " . $this->ecs->table('goods') . " SET " .
            ...
            if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list']))) {
                ...
                $sql = "SELECT attr_id, attr_index FROM " . $this->ecs->table('attribute') . " WHERE cat_id = '$goods_type'";
                ...
                FROM " . $this->ecs->table('goods_attr') . " AS g
                    LEFT JOIN " . $this->ecs->table('attribute') . " AS a
                ...
                $sql = "UPDATE " . $this->ecs->table('goods') . " SET keywords = '$keywords' WHERE goods_id = '$goods_id' LIMIT 1";
                ...
                foreach ($goods_attr_list as $attr_id => $attr_value_list) {
                    foreach ($attr_value_list as $attr_value => $info) {
                        if ($info['sign'] == 'insert') {
                            $sql = "INSERT INTO " . $this->ecs->table('goods_attr') . " (attr_id, goods_id, attr_value, attr_price)" .
                                "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]')";
                        } elseif ($info['sign'] == 'update') {
                            $sql = "UPDATE " . $this->ecs->table('goods_attr') . " SET attr_price = '$info[attr_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                        } else {
                            $sql = "DELETE FROM " . $this->ecs->table('goods_attr') . " WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                        }
                        $this->db->query($sql);
                    }
                }
            ...
            if ($goods_img !== false) {
                $this->db->query("UPDATE " . $this->ecs->table('goods') . " SET goods_img = '$goods_img' WHERE goods_id='$goods_id'");
            }

            if ($original_img !== false) {
                $this->db->query("UPDATE " . $this->ecs->table('goods') . " SET original_img = '$original_img' WHERE goods_id='$goods_id'");
            }
...
//-- 显示图片
        ...
        if ($_REQUEST['act'] == 'show_image') {
            ...
            $this->smarty->display('goods_show_image.htm');
        }
...
//-- 修改商品货号
        ...
        if ($_REQUEST['act'] == 'edit_goods_sn') {
            ...
            $sql = "SELECT goods_id FROM " . $this->ecs->table('products') . "WHERE product_sn='$goods_sn'";
        ...
        if ($_REQUEST['act'] == 'check_products_goods_sn') {
            ...
            if (!is_array($products_sn)) {
                ...
                foreach ($products_sn as $val) {
                    ...
                    $sql = "SELECT goods_id FROM " . $this->ecs->table('products') . "WHERE product_sn='$val'";
                    if ($this->db->getOne($sql)) {
                        make_json_error($val . $GLOBALS['_LANG']['goods_sn_exists']);
                    }
                }
...
//-- 排序、分页、查询
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch($tpl), '',
...
//-- 彻底删除商品
        ...
        if ($_REQUEST['act'] == 'drop_goods') {
            ...
            /* 删除商品的货品记录 */
            $sql = "DELETE FROM " . $this->ecs->table('products') .
                " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            ...
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('goods_attr') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('goods_cat') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('member_price') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('group_goods') . " WHERE parent_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('group_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('link_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('link_goods') . " WHERE link_goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('tag') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('comment') . " WHERE comment_type = 0 AND id_value = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('collect_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
            $sql = "DELETE FROM " . $this->ecs->table('booking_goods') . " WHERE goods_id = '$goods_id'";
            $this->db->query($sql);
...
//-- 把商品加入关联
        ...
        if ($_REQUEST['act'] == 'add_link_goods') {
            ...
            foreach ($linked_array as $val) {
                ...
                $sql = "INSERT INTO " . $this->ecs->table('link_goods') . " (goods_id, link_goods_id, is_double, admin_id) " .
                    "VALUES ('$goods_id', '$val', '$is_double', '" . session('admin_id') . "')";
                $this->db->query($sql, 'SILENT');
            }
...
//-- 删除关联商品
        ...
        if ($_REQUEST['act'] == 'drop_link_goods') {
            ...
            if (!$is_signle) {
                ...
                $sql = "UPDATE " . $this->ecs->table('link_goods') . " SET is_double = 0 " .
                    " WHERE link_goods_id = '$goods_id' AND goods_id " . $drop_goods_ids;
            }
            if ($goods_id == 0) {
...
//-- 增加一个配件
        ...
        if ($_REQUEST['act'] == 'add_group_goods') {
            ...
            foreach ($fittings as $val) {
                $sql = "INSERT INTO " . $this->ecs->table('group_goods') . " (parent_id, goods_id, goods_price, admin_id) " .
                    "VALUES ('$goods_id', '$val', '$price', '" . session('admin_id') . "')";
                $this->db->query($sql, 'SILENT');
            }
...
//-- 搜索文章
        ...
        if ($_REQUEST['act'] == 'get_article_list') {
            ...
            $sql = 'SELECT article_id, title FROM ' . $this->ecs->table('article') . $where .
...
//-- 添加关联文章
        ...
        if ($_REQUEST['act'] == 'add_goods_article') {
            ...
            foreach ($articles as $val) {
                $sql = "INSERT INTO " . $this->ecs->table('goods_article') . " (goods_id, article_id, admin_id) " .
                    "VALUES ('$goods_id', '$val', '" . session('admin_id') . "')";
                $this->db->query($sql);
            }
...
//-- 删除关联文章
        ...
        if ($_REQUEST['act'] == 'drop_goods_article') {
            ...
            $sql = "DELETE FROM " . $this->ecs->table('goods_article') . " WHERE " . db_create_in($articles, "article_id") . " AND goods_id = '$goods_id'";
            $this->db->query($sql);
...
//-- 货品列表
        ...
        if ($_REQUEST['act'] == 'product_list') {
            ...
            /* 取出商品信息 */
            $sql = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $this->ecs->table('goods') . " WHERE goods_id = '$goods_id'";
            $goods = $this->db->getRow($sql);
            if (empty($goods)) {
            ...
            $this->smarty->assign('action_link', array('href' => 'goods.php?act=list', 'text' => $GLOBALS['_LANG']['01_goods_list']));
...
//-- 货品排序、分页、查询
        ...
        if ($_REQUEST['act'] == 'product_query') {
            ...
            make_json_result($this->smarty->fetch('product_info.htm'), '',
...
//-- 货品删除
        ...
        if ($_REQUEST['act'] == 'product_remove') {
            ...
            /* 删除货品 */
            $sql = "DELETE FROM " . $this->ecs->table('products') . " WHERE product_id = '$product_id'";
...
//-- 修改货品价格
        ...
        if ($_REQUEST['act'] == 'edit_product_sn') {
            ...
            /* 修改 */
            $sql = "UPDATE " . $this->ecs->table('products') . " SET product_sn = '$product_sn' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
            if ($result) {
                clear_cache_files();
...
//-- 修改货品库存
        ...
        if ($_REQUEST['act'] == 'edit_product_number') {
            ...
            /* 修改货品库存 */
            $sql = "UPDATE " . $this->ecs->table('products') . " SET product_number = '$product_number' WHERE product_id = '$product_id'";
            $result = $this->db->query($sql);
            if ($result) {
                /* 修改商品库存 */
                if ($this->update_goods_stock($product['goods_id'], $product_number - $product['product_number'])) {
...
//-- 货品添加 执行
        ...
        if ($_REQUEST['act'] == 'product_add_execute') {
            ...
            /* 取出商品信息 */
            $sql = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $this->ecs->table('goods') . " WHERE goods_id = '" . $product['goods_id'] . "'";
            $goods = $this->db->getRow($sql);
            if (empty($goods)) {
                sys_msg($GLOBALS['_LANG']['sys']['wrong'] . $GLOBALS['_LANG']['cannot_found_goods'], 1, array(), false);
            }
        ...
        if ($_REQUEST['act'] == 'batch_product') {
            ...
            if ($_POST['type'] == 'drop') {
                ...
                if (!empty($product_array)) {
                    ...
                    /* 删除货品 */
                    $sql = "DELETE FROM " . $this->ecs->table('products') . " WHERE product_id $product_bound";
