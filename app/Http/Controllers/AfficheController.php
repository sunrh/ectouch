    ...
    public function actionIndex()
    {
        ...
        if ($_GET['act'] == 'js') {
            ...
            $sql = 'SELECT ad.ad_id, ad.ad_name, ad.ad_link, ad.ad_code ' .
                'FROM ' . $this->ecs->table('ad') . ' AS ad ' .
                'LEFT JOIN ' . $this->ecs->table('ad_position') . ' AS p ON ad.position_id = p.position_id ' .
            ...
            /* 如果是商品的站外JS */
            if ($ad_id == '-1') {
                $sql = "SELECT count(*) FROM " . $this->ecs->table('adsense') . " WHERE from_ad = '-1' AND referer = '" . $site_name . "'";
                if ($this->db->getOne($sql) > 0) {
                    $sql = "UPDATE " . $this->ecs->table('adsense') . " SET clicks = clicks + 1 WHERE from_ad = '-1' AND referer = '" . $site_name . "'";
                } else {
                    $sql = "INSERT INTO " . $this->ecs->table('adsense') . "(from_ad, referer, clicks) VALUES ('-1', '" . $site_name . "', '1')";
                }
                $this->db->query($sql);
                //$this->db->autoReplace($this->ecs->table('adsense'), array('from_ad' => -1, 'referer' => $site_name, 'clicks' => 1), array('clicks' => 1));
                $sql = "SELECT goods_name FROM " . $this->ecs->table('goods') . " WHERE goods_id = $goods_id";
                ...
                $sql = "SELECT count(*) FROM " . $this->ecs->table('adsense') . " WHERE from_ad = '" . $ad_id . "' AND referer = '" . $site_name . "'";
                if ($this->db->getOne($sql) > 0) {
                    $sql = "UPDATE " . $this->ecs->table('adsense') . " SET clicks = clicks + 1 WHERE from_ad = '" . $ad_id . "' AND referer = '" . $site_name . "'";
                } else {
                    $sql = "INSERT INTO " . $this->ecs->table('adsense') . "(from_ad, referer, clicks) VALUES ('" . $ad_id . "', '" . $site_name . "', '1')";
                }
                $this->db->query($sql);

                $sql = "SELECT * FROM " . $this->ecs->table('ad') . " WHERE ad_id = '$ad_id'";
                $ad_info = $this->db->getRow($sql);
                /* 跳转到广告的链接页面 */
                if (!empty($ad_info['ad_link'])) {
                    $uri = (strpos($ad_info['ad_link'], 'http://') === false && strpos($ad_info['ad_link'], 'https://') === false) ? $this->ecs->http() . urldecode($ad_info['ad_link']) : urldecode($ad_info['ad_link']);
                } else {
                    $uri = $this->ecs->url();
                }
