...
class CategoryController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $this->display = (isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array('list', 'grid', 'text'))) ? trim($_REQUEST['display']) : (request()->cookie('ectouch_display') ? request()->cookie('ectouch_display') : $default_display_type);
        $this->display = in_array($this->display, array('list', 'grid', 'text')) ? $this->display : 'text';
        cookie()->queue('ectouch_display', $this->display, 1440 * 7);

        /* 页面的缓存ID */
        $cache_id = sprintf('%X', crc32($cat_id . '-' . $this->display . '-' . $sort . '-' . $order . '-' . $page . '-' . $size . '-' . session('user_rank') . '-' .
        ...
        if (!$this->smarty->is_cached('category.dwt', $cache_id)) {
            ...
            if ($cat['filter_attr'] > 0) {
                ...
                foreach ($cat_filter_attr as $key => $value) {
                    $sql = "SELECT a.attr_name FROM " . $this->ecs->table('attribute') . " AS a, " . $this->ecs->table('goods_attr') . " AS ga, " . $this->ecs->table('goods') . " AS g WHERE ($children OR " . get_extension_goods($children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND a.attr_id='$value'";
                    if ($temp_name = $this->db->getOne($sql)) {
                        $all_attr_list[$key]['filter_attr_name'] = $temp_name;

                        $sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $this->ecs->table('goods_attr') . " AS a, " . $this->ecs->table('goods') .
                            " AS g" .
                ...
                if (!empty($filter_attr)) {
                    ...
                    foreach ($filter_attr as $k => $v) {                      // 查出符合所有筛选属性条件的商品id */
                        if (is_numeric($v) && $v != 0 && isset($cat_filter_attr[$k])) {
                            ...
                            $ext_group_goods = $this->db->getColCached($sql);
            ...
            $this->smarty->assign('categories', get_categories_tree($cat_id)); // 分类树
            ...
            $this->smarty->assign('brand_id', $brand);
            ...
            $this->smarty->assign('feed_url', ($GLOBALS['_CFG']['rewrite'] == 1) ? "feed-c$cat_id.xml" : 'feed.php?cat=' . $cat_id); // RSS URL
            ...
            $this->smarty->assign('best_goods', get_category_recommend_goods('best', $children, $brand, $price_min, $price_max, $ext));
            $this->smarty->assign('promotion_goods', get_category_recommend_goods('promote', $children, $brand, $price_min, $price_max, $ext));
            $this->smarty->assign('hot_goods', get_category_recommend_goods('hot', $children, $brand, $price_min, $price_max, $ext));
            ...
            $this->smarty->assign('category', $cat_id);
            $this->smarty->assign('script_name', 'category');

            assign_pager('category', $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max, $this->display, $filter_attr_str); // 分页
            assign_dynamic('category'); // 动态内容
        }

        $this->smarty->display('category.dwt', $cache_id);
    }
