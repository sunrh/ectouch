    ...
    public function actionIndex()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if (!$this->smarty->is_cached('exchange.dwt', $cache_id)) {
                ...
                //$this->smarty->assign('best_goods', $this->get_exchange_recommend_goods('best', $children, $integral_min, $integral_max));
                //$this->smarty->assign('new_goods', $this->get_exchange_recommend_goods('new',  $children, $integral_min, $integral_max));
                $this->smarty->assign('hot_goods', $this->get_exchange_recommend_goods('hot', $children, $integral_min, $integral_max));
                ...
                assign_pager('exchange', $cat_id, $count, $size, $sort, $order, $page, '', '', $integral_min, $integral_max, $this->display); // 分页
                assign_dynamic('exchange_list'); // 动态内容
            }

            $this->smarty->assign('feed_url', ($GLOBALS['_CFG']['rewrite'] == 1) ? "feed-typeexchange.xml" : 'feed.php?type=exchange'); // RSS URL
            $this->smarty->display('exchange_list.dwt', $cache_id);
        }
        ...
        if ($_REQUEST['act'] == 'view') {
            ...
            if (!$this->smarty->is_cached('exchange_goods.dwt', $cache_id)) {
                $this->smarty->assign('image_width', $GLOBALS['_CFG']['image_width']);
                $this->smarty->assign('image_height', $GLOBALS['_CFG']['image_height']);
                $this->smarty->assign('helps', get_shop_help()); // 网店帮助
                ...
                if ($goods === false) {
                    ...
                    if (!empty($prev_gid)) {
                        ...
                        $this->smarty->assign('prev_good', $prev_good);//上一个商品
                    }

                    $sql = "SELECT max(eg.goods_id) FROM " . $this->ecs->table('exchange_goods') . " AS eg," . $GLOBALS['ecs']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id < " . $goods['goods_id'] . " AND eg.is_exchange = 1 AND g.is_delete = 0";
                    $next_gid = $this->db->getOne($sql);
                    if (!empty($next_gid)) {
                    ...
                    $this->smarty->assign('pictures', get_goods_gallery($goods_id));                    // 商品相册

                    assign_dynamic('exchange_goods');
                }
            }

            $this->smarty->display('exchange_goods.dwt', $cache_id);
        }
