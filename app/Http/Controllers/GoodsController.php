...
class GoodsController extends Controller
{
    public function actionIndex()
    {
        ...
        if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'gotopage') {
            ...
            if (!empty($goods_id)) {
                ...
                $bought_notes = $this->db->getAll($sql);
        ...
        $cache_id = $goods_id . '-' . session('user_rank') . '-' . $GLOBALS['_CFG']['lang'];
        $cache_id = sprintf('%X', crc32($cache_id));
        if (!$this->smarty->is_cached('goods.dwt', $cache_id)) {
            ...
            if ($goods === false) {
                ...
                $next_gid = $this->db->getOne("SELECT max(goods_id) FROM " . $this->ecs->table('goods') . " WHERE cat_id=" . $goods['cat_id'] . " AND goods_id < " . $goods['goods_id'] . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0");
                if (!empty($next_gid)) {
                ...
                $this->smarty->assign('attribute_linked', get_same_attribute_goods($properties));           // 相同属性的关联商品
                $this->smarty->assign('related_goods', $linked_goods);                                   // 关联商品
                $this->smarty->assign('goods_article_list', $this->get_linked_articles($goods_id));                  // 关联文章
                $this->smarty->assign('fittings', get_goods_fittings(array($goods_id)));                   // 配件
                $this->smarty->assign('rank_prices', $this->get_user_rank_prices($goods_id, $shop_price));    // 会员等级价格
                $this->smarty->assign('pictures', get_goods_gallery($goods_id));                    // 商品相册
                $this->smarty->assign('bought_goods', $this->get_also_bought($goods_id));                      // 购买了该商品的用户还购买了哪些商品
                $this->smarty->assign('goods_rank', $this->get_goods_rank($goods_id));                       // 商品的销售排名
        ...
        $this->smarty->display('goods.dwt', $cache_id);
    }
    ...
    private function get_package_goods_list($goods_id)
    {
        ...
        foreach ($res as $tempkey => $value) {
            ...
            $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, p.goods_attr, g.goods_sn, g.goods_name, g.market_price, g.goods_thumb, IFNULL(mp.user_price, g.shop_price * '". session('discount') ."') AS rank_price
