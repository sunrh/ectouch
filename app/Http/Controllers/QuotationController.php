...
class QuotationController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($action == 'print_quotation') {
            ...
            $this->smarty->assign('shop_name', $GLOBALS['_CFG']['shop_title']);
            ...
            $sql = "SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_number, c.cat_name AS goods_category,p.product_id,p.product_number,p.goods_attr" .
                " FROM " . $this->ecs->table('goods') . " AS g LEFT JOIN " . $this->ecs->table('category') . " AS c ON g.cat_id = c.cat_id LEFT JOIN " . $this->ecs->table('products') . "as p  On g.goods_id=p.goods_id" . $where . " AND is_on_sale = 1 AND is_alone_sale = 1 ";
            ...
            $user_rank = $this->db->getAll("SELECT * FROM " . $this->ecs->table('user_rank') . "WHERE show_price = 1 OR rank_id = '". session('user_rank') ."'");
            $rank_point = 0;
            if (!empty(session('user_id'))) {
                $rank_point = $this->db->getOne("SELECT rank_points FROM " . $this->ecs->table('users') . " WHERE user_id = '" . session('user_id') . "'");
            }
            $user_rank = $this->calc_user_rank($user_rank, $rank_point);
            $user_men = $this->serve_user($goods_list);
            $this->smarty->assign('extend_price', $user_rank['ext_price']);
            $this->smarty->assign('extend_rank', $user_men);
            $this->smarty->assign('goods_list', $goods_list);

            $html = $this->smarty->fetch('quotation_print.html');
            exit($html);
        }
        ...
        if (is_null($this->smarty->get_template_vars('helps'))) {
            $this->smarty->assign('helps', get_shop_help()); // 网店帮助
        }

        $this->smarty->display('quotation.dwt');
    }
