    ...
    public function actionIndex()
    {
        ...
        if ($act == 'menu_api') {
            ...
            if ($api_data === false || (isset($api_data['api_time']) && $api_data['api_time'] < date('Ymd'))) {
                ...
                $api_comment = $t->request('http://cloud.ecmoban.com/menu_api.php', $apiget);
            ...
            if ($api_data === false || (isset($api_data['api_time']) && $api_data['api_time'] < date('Ymd'))) {
                ...
                $api_comment = $t->request('http://cloud.ecmoban.com/cloud_remind.php', $apiget);
                $api_str = $api_comment["body"];
                $json = new Json();
                $api_arr = @$json->decode($api_str, 1);
                if (!empty($api_str)) {
                    if (!empty($api_arr) && $api_arr['error'] == 0 && md5($api_arr['content']) == $api_arr['hash']) {
                        ...
                        $api_arr['content'] = '<li  class="cloud_close">' . $message['0'] . '<img onclick="cloud_close(' . $message['1'] . ')" src="'. __TPL__ .'/images/no.gif"></li>';
                        if ($data['charset'] != 'UTF-8') {
                            $api_arr['content'] = ecs_iconv('UTF-8', $data['charset'], $api_arr['content']);
                        }
                        $api_arr['api_time'] = date('Ymd');
            ...
            if (!empty($api_str)) {
                if (!empty($api_arr) && $api_arr['error'] == 0 && md5($api_arr['content']) == $api_arr['hash']) {
                    ...
                    if (admin_priv('all', '', false)) {
                        ...
                        $result = $t->request('http://cloud.ecmoban.com/cloud_remind.php', $apiget);
                        $api_str = $result["body"];
                        //var_dump($api_str);
                        $api_arr = array();
                        $api_arr = @$json->decode($api_str, 1);
                        ...
                        $api_arr['content'] = '<li  class="cloud_close">' . $message['0'] . '&nbsp;&nbsp;&nbsp;&nbsp;' . $GLOBALS['_LANG']['cloud_no_priv'] . '<img onclick="cloud_close( ' . $message['1'] . ')" src="'. __TPL__ .'/images/no.gif"></li>';
            ...
            ecs_header("Location: http://cloud.ecmoban.com/api.php?act=" . $act . $query . "\n");
            exit();
        }
    }
}