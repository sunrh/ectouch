...
//-- 红包类型列表页面
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['bonustype_add'], 'href' => 'bonus.php?act=add'));
            ...
            $this->smarty->display('bonus_type.htm');
        }
...
//-- 翻页、排序
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('bonus_type.htm'), '',
...
//-- 红包类型添加页面
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $this->smarty->assign('action_link', array('href' => 'bonus.php?act=list', 'text' => $GLOBALS['_LANG']['04_bonustype_list']));
            ...
            $this->smarty->display('bonus_type_info.htm');
        }
...
//-- 红包类型编辑页面
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            $this->smarty->assign('action_link', array('href' => 'bonus.php?act=list&' . list_link_postfix(), 'text' => $GLOBALS['_LANG']['04_bonustype_list']));
...
//-- 红包类型编辑的处理
        ...
        if ($_REQUEST['act'] == 'update') {
            ...
            $sql = "UPDATE " . $this->ecs->table('bonus_type') . " SET " .
                "type_name       = '$type_name', " .
...
//-- 红包发送页面
        ...
        if ($_REQUEST['act'] == 'send') {
            ...
            if ($_REQUEST['send_by'] == SEND_BY_USER) {
                ...
                $this->smarty->display('bonus_by_user.htm');
...
//-- 按印刷品发放红包
        ...
        if ($_REQUEST['act'] == 'send_by_print') {
            ...
            for ($i = 0, $j = 0; $i < $bonus_sum; $i++) {
                ...
                $this->db->query("INSERT INTO " . $this->ecs->table('user_bonus') . " (bonus_type_id, bonus_sn) VALUES('$bonus_typeid', '$bonus_sn')");
...
//-- 导出线下发放的信息
        ...
        if ($_REQUEST['act'] == 'gen_excel') {
            ...
            $sql = "SELECT ub.bonus_id, ub.bonus_type_id, ub.bonus_sn, bt.type_name, bt.type_money, bt.use_end_date " .
                "FROM " . $this->ecs->table('user_bonus') . " AS ub, " . $this->ecs->table('bonus_type') . " AS bt " .
...
//-- 添加发放红包的商品
        ...
        if ($_REQUEST['act'] == 'add_bonus_goods') {
            ...
            foreach ($add_ids as $key => $val) {
                $sql = "UPDATE " . $this->ecs->table('goods') . " SET bonus_type_id='$type_id' WHERE goods_id='$val'";
                $this->db->query($sql, 'SILENT') or make_json_error($this->db->error());
            }
...
//-- 搜索用户
        ...
        if ($_REQUEST['act'] == 'search_users') {
            ...
            $sql = "SELECT user_id, user_name FROM " . $this->ecs->table('users') .
...
//-- 红包列表
        ...
        if ($_REQUEST['act'] == 'bonus_list') {
            ...
            $this->smarty->display('bonus_list.htm');
        }
...
//-- 红包列表翻页、排序
        ...
        if ($_REQUEST['act'] == 'query_bonus') {
            ...
            make_json_result($this->smarty->fetch('bonus_list.htm'), '',
                array('filter' => $list['filter'], 'page_count' => $list['page_count']));
        }
...
//-- 删除红包
        ...
        if ($_REQUEST['act'] == 'remove_bonus') {
            ...
            $this->db->query("DELETE FROM " . $this->ecs->table('user_bonus') . " WHERE bonus_id='$id'");
