 ...
 */
class AffiliateCkController extends Controller
{
...
//-- 分成页
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if (!empty($_GET['auid'])) {
                $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['back_note'], 'href' => "users.php?act=edit&id=$_GET[auid]"));
            }

            $this->smarty->display('affiliate_ck_list.htm');
        }
        /*------------------------------------------------------ */
//-- 分页
        /*------------------------------------------------------ */
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('affiliate_ck_list.htm'), '', array('filter' => $logdb['filter'], 'page_count' => $logdb['page_count']));
        }
        /*
            取消分成，不再能对该订单进行分成
        */
        if ($_REQUEST['act'] == 'del') {
            ...
            $stat = $this->db->getOne("SELECT is_separate FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE order_id = '$oid'");
            if (empty($stat)) {
        ...
        */
        if ($_REQUEST['act'] == 'rollback') {
            ...
            $stat = $this->db->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('affiliate_log') . " WHERE log_id = '$logid'");
            if (!empty($stat)) {
        ...
        */
        if ($_REQUEST['act'] == 'separate') {
            ...
            $row = $this->db->getRow("SELECT o.order_sn, o.is_separate, (o.goods_amount - o.discount) AS goods_amount, o.user_id FROM " . $GLOBALS['ecs']->table('order_info') . " o" .
