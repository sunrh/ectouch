...
class CategoryController extends Controller
{
    public function actionIndex()
    {
        ...
        $exc = new Exchange($this->ecs->table("category"), $this->db, 'cat_id', 'cat_name');
...
//-- 商品分类列表
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->assign('action_link', array('href' => 'category.php?act=add', 'text' => $GLOBALS['_LANG']['04_category_add']));
            ...
            $this->smarty->display('category_list.htm');
        }
...
//-- 排序、分页、查询
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('category_list.htm'));
        }
        /*------------------------------------------------------ */
//-- 添加商品分类
        /*------------------------------------------------------ */
        if ($_REQUEST['act'] == 'add') {
            ...
            $this->smarty->assign('action_link', array('href' => 'category.php?act=list', 'text' => $GLOBALS['_LANG']['03_category_list']));

            $this->smarty->assign('goods_type_list', goods_type_list(0)); // 取得商品类型
            $this->smarty->assign('attr_list', $this->get_attr_list()); // 取得商品属性

            $this->smarty->assign('cat_select', cat_list(0, 0, true));
            $this->smarty->assign('form_act', 'insert');
            $this->smarty->assign('cat_info', array('is_show' => 1));


            /* 显示页面 */

            $this->smarty->display('category_info.htm');
        }
...
//-- 商品分类添加时的处理
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            /* 入库的操作 */
            if ($this->db->autoExecute($this->ecs->table('category'), $cat) !== false) {
                ...
                if ($cat['show_in_nav'] == 1) {
                    ...
                    $sql = "INSERT INTO " . $this->ecs->table('nav') .
                        ...
                        " VALUES('" . $cat['cat_name'] . "', 'c', '" . $this->db->insert_id() . "','1','$vieworder','0', '" . build_uri('category', array('cid' => $cat_id), $cat['cat_name']) . "','middle')";
                    $this->db->query($sql);
                }
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            if ($cat_info['filter_attr']) {
                ...
                foreach ($filter_attr as $k => $v) {
                    $attr_cat_id = $this->db->getOne("SELECT cat_id FROM " . $this->ecs->table('attribute') . " WHERE attr_id = '" . intval($v) . "'");
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['03_category_list'], 'href' => 'category.php?act=list'));

            //分类是否存在首页推荐
            $res = $this->db->getAll("SELECT recommend_type FROM " . $this->ecs->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
            if (!empty($res)) {
                $cat_recommend = array();
                foreach ($res as $data) {
            ...
            $this->smarty->assign('cat_select', cat_list(0, $cat_info['parent_id'], true));
            $this->smarty->assign('goods_type_list', goods_type_list(0)); // 取得商品类型
...
//-- 编辑商品分类信息
        ...
        if ($_REQUEST['act'] == 'update') {
            ...
            $dat = $this->db->getRow("SELECT cat_name, show_in_nav FROM " . $this->ecs->table('category') . " WHERE cat_id = '$cat_id'");

            if ($this->db->autoExecute($this->ecs->table('category'), $cat, 'UPDATE', "cat_id='$cat_id'")) {
                if ($cat['cat_name'] != $dat['cat_name']) {
                    //如果分类名称发生了改变
                    $sql = "UPDATE " . $this->ecs->table('nav') . " SET name = '" . $cat['cat_name'] . "' WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
                    $this->db->query($sql);
                }
                if ($cat['show_in_nav'] != $dat['show_in_nav']) {
                    ...
                    if ($cat['show_in_nav'] == 1) {
                        //显示
                        $nid = $this->db->getOne("SELECT id FROM " . $this->ecs->table('nav') . " WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                        if (empty($nid)) {
                            //不存在
                            $vieworder = $this->db->getOne("SELECT max(vieworder) FROM " . $this->ecs->table('nav') . " WHERE type = 'middle'");
                            $vieworder += 2;
                            ...
                            $sql = "INSERT INTO " . $this->ecs->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $cat['cat_name'] . "', 'c', '$cat_id','1','$vieworder','0', '" . $uri . "','middle')";
                        } else {
                            $sql = "UPDATE " . $this->ecs->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
                        }
                        $this->db->query($sql);
                    } else {
                        //去除
                        $this->db->query("UPDATE " . $this->ecs->table('nav') . " SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                    }
                }
...
//-- 批量转移商品分类页面
        ...
        if ($_REQUEST['act'] == 'move') {
            ...
            $this->smarty->assign('cat_select', cat_list(0, $cat_id, true));
            $this->smarty->assign('form_act', 'move_cat');

            /* 显示页面 */

            $this->smarty->display('category_move.htm');
        }
...
//-- 处理批量转移商品分类的处理程序
        ...
        if ($_REQUEST['act'] == 'move_cat') {
            ...
            /* 更新商品分类 */
            $sql = "UPDATE " . $this->ecs->table('goods') . " SET cat_id = '$target_cat_id' " .
                "WHERE cat_id = '$cat_id'";
        ...
        if ($_REQUEST['act'] == 'toggle_show_in_nav') {
            ...
            if ($this->cat_update($id, array('show_in_nav' => $val)) != false) {
                if ($val == 1) {
                    ...
                    $vieworder = $this->db->getOne("SELECT max(vieworder) FROM " . $this->ecs->table('nav') . " WHERE type = 'middle'");
                    $vieworder += 2;
                    $catname = $this->db->getOne("SELECT cat_name FROM " . $this->ecs->table('category') . " WHERE cat_id = '$id'");
                    //显示在自定义导航栏中
                    $GLOBALS['_CFG']['rewrite'] = 0;
                    $uri = build_uri('category', array('cid' => $id), $catname);

                    $nid = $this->db->getOne("SELECT id FROM " . $this->ecs->table('nav') . " WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'");
                    if (empty($nid)) {
                        //不存在
                        $sql = "INSERT INTO " . $this->ecs->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $catname . "', 'c', '$id','1','$vieworder','0', '" . $uri . "','middle')";
                    } else {
                        $sql = "UPDATE " . $this->ecs->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'";
                    }
                    $this->db->query($sql);
                } else {
                    //去除
                    $this->db->query("UPDATE " . $this->ecs->table('nav') . "SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'");
                }
                clear_cache_files();
                make_json_result($val);
            } else {
...
//-- 删除商品分类
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            $cat_name = $this->db->getOne('SELECT cat_name FROM ' . $this->ecs->table('category') . " WHERE cat_id='$cat_id'");

            /* 当前分类下是否有子分类 */
            $cat_count = $this->db->getOne('SELECT COUNT(*) FROM ' . $this->ecs->table('category') . " WHERE parent_id='$cat_id'");

            /* 当前分类下是否存在商品 */
            $goods_count = $this->db->getOne('SELECT COUNT(*) FROM ' . $this->ecs->table('goods') . " WHERE cat_id='$cat_id'");

            /* 如果不存在下级子分类和商品，则删除之 */
            if ($cat_count == 0 && $goods_count == 0) {
                /* 删除分类 */
                $sql = 'DELETE FROM ' . $this->ecs->table('category') . " WHERE cat_id = '$cat_id'";
                if ($this->db->query($sql)) {
                    $this->db->query("DELETE FROM " . $this->ecs->table('nav') . "WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                    clear_cache_files();
...
// * @return  boolean
// */
//private function cat_exists($cat_name, $parent_cat, $exclude = 0)
//{
