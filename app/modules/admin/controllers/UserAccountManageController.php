...
//--商品明细列表
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $sql = " SELECT IFNULL(SUM(user_money), 0) AS user_money, IFNULL(SUM(frozen_money), 0) AS frozen_money FROM " .
                $this->ecs->table('account_log') . " WHERE `change_time` >= " . $start_date . " AND `change_time` < " . ($end_date + 86400);
            ...
            $sql = "SELECT IFNULL(SUM(surplus), 0) AS surplus, IFNULL(SUM(integral_money), 0) AS integral_money FROM " .
                $this->ecs->table('order_info') . " WHERE 1 AND `add_time` >= " . $start_date . " AND `add_time` < " . ($end_date + 86400);
            $money_list = $this->db->getRow($sql);
            ...
            $this->smarty->display('user_account_manage.htm');
        }
        if ($_REQUEST['act'] == 'surplus') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['user_account_manage'], 'href' => 'user_account_manage.php?act=list&start_date=' . local_date('Y-m-d', $start_date) . '&end_date=' . local_date('Y-m-d', $end_date)));

            /* 显示页面 */

            $this->smarty->display('order_surplus_list.htm');
        }
...
//-- ajax返回用户列表
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('order_surplus_list.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
        }
    }
