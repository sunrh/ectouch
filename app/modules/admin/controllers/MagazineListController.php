...
class MagazineListController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['add_new'], 'href' => 'magazine_list.php?act=add'));
            ...
            $this->smarty->display('magazine_list.htm');
        }
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('magazine_list.htm'), '', array('filter' => $magazinedb['filter'], 'page_count' => $magazinedb['page_count']));
        }
        if ($_REQUEST['act'] == 'add') {
            if (empty($_POST['step'])) {
                include_once(ROOT_PATH . 'includes/fckeditor/fckeditor.php'); // 包含 html editor 类文件
                $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['go_list'], 'href' => 'magazine_list.php?act=list'));
                $this->smarty->assign(array('ur_here' => $GLOBALS['_LANG']['magazine_list'], 'act' => 'add'));
                create_html_editor('magazine_content');

                $this->smarty->display('magazine_list_add.htm');
            } elseif ($_POST['step'] == 2) {
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            if (empty($_POST['step'])) {
                ...
                $this->smarty->assign(array('id' => $id, 'act' => 'edit', 'magazine_name' => $rt['template_subject'], 'magazine_content' => $rt['template_content']));
                $this->smarty->assign(array('ur_here' => $GLOBALS['_LANG']['magazine_list'], 'act' => 'edit'));
                $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['go_list'], 'href' => 'magazine_list.php?act=list'));
