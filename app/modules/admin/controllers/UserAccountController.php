...
//-- 会员余额记录列表
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if (isset($_REQUEST['is_paid'])) {
                $this->smarty->assign('is_paid_' . intval($_REQUEST['is_paid']), 'selected="selected"');
            }
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['surplus_add'], 'href' => 'user_account.php?act=add'));
            ...
            $this->smarty->display('user_account_list.htm');
        }
...
//-- 添加/编辑会员余额页面
        ...
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            $this->smarty->assign('action_link', array('href' => $href, 'text' => $GLOBALS['_LANG']['09_user_account']));


            $this->smarty->display('user_account_info.htm');
        }
...
//-- 添加/编辑会员余额的处理部分
        ...
        if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                $sql = "INSERT INTO " . $this->ecs->table('user_account') .
                    " VALUES ('', '$user_id', '" . session('admin_name') . "', '$amount', '" . gmtime() . "', '" . gmtime() . "', '$admin_note', '$user_note', '$process_type', '$payment', '$is_paid')";
                $this->db->query($sql);
...
//-- 审核会员余额页面
        ...
        if ($_REQUEST['act'] == 'check') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['09_user_account'],
                'href' => 'user_account.php?act=list&' . list_link_postfix()));

            /* 页面显示 */

            $this->smarty->display('user_account_check.htm');
        }
...
//-- ajax帐户信息列表
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('user_account_list.htm'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
        }
        /*------------------------------------------------------ */
//-- ajax删除一条信息
        /*------------------------------------------------------ */
        if ($_REQUEST['act'] == 'remove') {
            ...
            $sql = "SELECT u.user_name FROM " . $this->ecs->table('users') . " AS u, " .
                $this->ecs->table('user_account') . " AS ua " .
                " WHERE u.user_id = ua.user_id AND ua.id = '$id' ";
            $user_name = $this->db->getOne($sql);
            $sql = "DELETE FROM " . $this->ecs->table('user_account') . " WHERE id = '$id'";
