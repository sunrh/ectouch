...
class SearchController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($_REQUEST['act'] == 'advanced_search') {
            ...
            $this->smarty->assign('cat_list', cat_list(0, 0, true, 2, false));
            ...
            $this->smarty->display('search.dwt');
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'form') {
            ...
            $this->smarty->assign('goods_type_list', $attributes['cate']);
            $this->smarty->assign('goods_attributes', $attributes['attr']);
            $this->smarty->assign('goods_type_selected', $_REQUEST['goods_type']);
            $this->smarty->assign('cat_list', cat_list(0, $adv_value['category'], true, 2, false));
            $this->smarty->assign('brand_list', get_brand_list());
            $this->smarty->assign('action', 'form');
            $this->smarty->assign('use_storage', $GLOBALS['_CFG']['use_storage']);
        ...
        if (!empty($_REQUEST['keywords'])) {
            ...
            foreach ($arr as $key => $val) {
                ...
                $this->db->autoReplace($this->ecs->table('keywords'), array('date' => local_date('Y-m-d'),
        ...
        $display = (isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array('list', 'grid', 'text'))) ? trim($_REQUEST['display']) : session('display_search', $default_display_type);

        session(['display_search' => $display]);
        ...
        if (!empty($_REQUEST['attr'])) {
            $sql = "SELECT goods_id, COUNT(*) AS num FROM " . $this->ecs->table("goods_attr") . " WHERE 0 ";
        ...
        $this->smarty->assign('keywords', htmlspecialchars(stripslashes($_REQUEST['keywords'])));
        $this->smarty->assign('search_keywords', stripslashes(htmlspecialchars_decode($_REQUEST['keywords'])));
        $this->smarty->assign('brand', $_REQUEST['brand']);
