...
class Goods extends Foundation
{
    ...
    public static function getBestGoodsList(array $attributes)
    {
        ...
        $total = self::find()
            ->where(['is_best' => 1, 'is_delete' => 0, 'is_on_sale' => 1])
            ->count();
        $pages = new Pagination(['totalCount' => $total, 'pageSize' => $per_page, 'page' => $page-1]);
    ...
    public static function getInfo(array $attributes)
    {
        ...
        //商品的虚拟销量
        $goods_virtual_sales = $model
            ->select(['virtual_sales'])
            ->asArray()
            ->one();
       
        //商品销量 = 商品销量 + 虚拟销量
        $data['sales'] = intval($goods_virtual_sales['virtual_sales']) + intval($data['sales']);
        ...
        $pattern = '/<img(.*?)src=\"\/(.*?)\"(.*?)>/i';
        $replacement = '<img$1src="'. Yii::$app->params['SHOP_URL'] .'/$2"$3>';
        $data['goods_desc'] = preg_replace($pattern, $replacement, $data['goods_desc']);
        ...
        $data['companyName'] = ShopConfig::findByCode('shop_name')->value;
        if (!$data) {
     ...
     * 属性选择  返回商品总价
     */
    public static function getPropertyTotal(array $attributes)
    {
        extract($attributes);

        if (isset($good_property) && !empty($good_property)) {
            $data = Goods::get_final_price($goods_id, $goods_number, true, $good_property);
        } else {
            $data = Goods::get_final_price($goods_id, $goods_number);
        }
        $total = $data * $goods_number;
        return self::formatBody(['total_price' => Goods::price_format($total, false)]);
    }
