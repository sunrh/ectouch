...
class Template
{
    ...
    {
        $suffix = str_replace('/', '.', $GLOBALS['_CFG']['template']);
        $name = $this->compile_dir . '/' . basename($filename) . '.' . $suffix . '.php';
        if ($this->_expires) {
...
//            }
        ...
        foreach ($arr as $val) {
            if (in_array($val, $scripts) == false) {
                ...
                if ($val{0} == '.') {
                    ...
                    $str .= '<script type="text/javascript" src="' . asset((defined('ECS_ADMIN') ? 'vendor/admin/' : '') . 'js/' . $val) . '"></script>' . "\n";
                }
            }
        }
    ...
    public function smarty_prefilter_preCompile($source)
    {
        ...
        $tmp_dir = asset('themes/' . $GLOBALS['_CFG']['template']) . '/'; // 模板所在路径
