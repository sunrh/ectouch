...
function add_feed($id, $feed_type)
{
    ...
    if ($feed_type == BUY_GOODS) {
        ...
        foreach ($order_res as $goods_data) {
            ...
            $feed['title_data'] = array('username' => session('user_name'), 'goods_name' => $goods_data['goods_name']);
            ...
            $feed['images'][] = array('url' => $url,
                'link' => $link);
            uc_call("uc_feed_add", array($feed['icon'], session('user_id'), session('user_name'), $feed['title_template'], $feed['title_data'], $feed ['body_template'], $feed['body_data'], '', '', $feed['images']));
        }
    }
    return;
}
