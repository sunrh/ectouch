...
// ECM 订单状态
    ...
    public static function getList(array $attributes)
    {
        ...
        if (!$uid) {
            return self::formatBody(['orders' => [],'paged' => self::formatPaged($page, $per_page, 0)]);
        }
        ...
        $data = $model
            ->select(['order_id', 'order_sn', 'order_status', 'shipping_status', 'pay_status','goods_amount','order_amount','add_time','shipping_status','shipping_status', 'money_paid'])
            ->with('goods')
    ...
    public static function confirm(array $attributes)
    {
        ...
        if ($model = self::find()
            ...
            ->andWhere(['in', 'shipping_status', [self::SS_SHIPPED, self::SS_SHIPPED_PART, self::OS_SHIPPED_PART]])
            ->one()
        ) {
            //修改订单状态
            $model->shipping_status = self::SS_RECEIVED;
            if ($model->save(false)) {
                OrderAction::toCreateOrUpdate($model->order_id, $model->order_status, self::SS_RECEIVED, $model->pay_status, '订单确认');
    ...
    public static function getInfo(array $attributes)
    {
        ...
        $orderInfo = self::find()
            ->select(['order_id', 'order_sn', 'shipping_id', 'shipping_name', 'pay_id', 'pay_name', 'shipping_fee', 'goods_amount', 'order_amount', 'consignee',  'district', 'address', 'mobile', 'add_time', 'postscript as comment', 'order_status', 'pay_status', 'shipping_status', 'money_paid'])
            ->where(['user_id' => $uid, 'order_id' => $order])
            ->asArray()
            ->one();
