...
/**
 * Class EditLanguagesController
 * @package app\modules\admin\controllers
 */
class EditLanguagesController extends Controller
{
...
//-- 列表编辑 ?act=list
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            /* 模板赋值 */
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['edit_languages']);
            $this->smarty->assign('keyword', $keyword);  //关键字
            $this->smarty->assign('action_link', array());
            $this->smarty->assign('file_attr', $file_attr);//文件权限
            $this->smarty->assign('lang_arr', $lang_arr); //语言文件列表
            $this->smarty->assign('file_path', $file_path);//语言文件
            $this->smarty->assign('lang_file', $lang_file);//语言文件
            $this->smarty->assign('language_arr', $language_arr); //需要编辑的语言项列表


            $this->smarty->display('language_list.htm');
        }
