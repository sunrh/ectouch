...
class PackController extends Controller
{
    public function actionIndex()
    {
        ...
        $exc = new Exchange($this->ecs->table("pack"), $this->db, 'pack_id', 'pack_name');
...
//-- 包装列表
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['pack_add'], 'href' => 'pack.php?act=add'));
            ...
            $this->smarty->display('pack_list.htm');
        }
        /*------------------------------------------------------ */
//-- ajax 列表
        /*------------------------------------------------------ */
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('pack_list.htm'), '', array('filter' => $packs_list['filter'], 'page_count' => $packs_list['page_count']));
        }
        /*------------------------------------------------------ */
//-- 添加新包装
        /*------------------------------------------------------ */
        if ($_REQUEST['act'] == 'add') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['06_pack_list'], 'href' => 'pack.php?act=list'));


            $this->smarty->display('pack_info.htm');
        }
        if ($_REQUEST['act'] == 'insert') {
            ...
            /*插入数据*/
            $sql = "INSERT INTO " . $this->ecs->table('pack') . "(pack_name, pack_fee, free_money, pack_desc, pack_img)
...
//-- 编辑包装
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['06_pack_list'], 'href' => 'pack.php?act=list&' . list_link_postfix()));
            $this->smarty->assign('pack', $pack);
        ...
        if ($_REQUEST['act'] == 'drop_pack_img') {
            ...
            if (!empty($img_name)) {
                ...
                $sql = "UPDATE " . $this->ecs->table('pack') . " SET pack_img = '' WHERE pack_id = '$pack_id'";
                $this->db->query($sql);
            }
