...
class NavigatorController extends Controller
{
    public function actionIndex()
    {
        ...
        $exc = new Exchange($this->ecs->table("nav"), $this->db, 'id', 'name');
...
//-- 自定义导航栏列表
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['add_new'], 'href' => 'navigator.php?act=add'));
            ...
            $this->smarty->display('navigator.htm');
        }
        /*------------------------------------------------------ */
//-- 自定义导航栏列表Ajax
        /*------------------------------------------------------ */
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('navigator.htm'), '', array('filter' => $navdb['filter'], 'page_count' => $navdb['page_count']));
        }
        /*------------------------------------------------------ */
//-- 自定义导航栏增加
        /*------------------------------------------------------ */
        if ($_REQUEST['act'] == 'add') {
            if (empty($_REQUEST['step'])) {
                ...
                $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['go_list'], 'href' => 'navigator.php?act=list'));
                $this->smarty->assign('ur_here', $GLOBALS['_LANG']['navigator']);

                $this->smarty->assign('sysmain', $sysmain);
                $this->smarty->assign('rt', $rt);
                $this->smarty->display('navigator_add.htm');
                ...
                $vieworder = $this->db->getOne("SELECT max(vieworder) FROM " . $this->ecs->table('nav') . " WHERE type = '" . $item_type . "'");
...
//-- 自定义导航栏编辑
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            if (empty($_REQUEST['step'])) {
                ...
                $row = $this->db->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('nav') . " WHERE id='$id'");
                ...
                $row = $this->db->getRow("SELECT ctype,cid,ifshow,type FROM " . $GLOBALS['ecs']->table('nav') . " WHERE id = '$id'");
...
//-- 自定义导航栏删除
        ...
        if ($_REQUEST['act'] == 'del') {
            $id = (int)$_GET['id'];
            $row = $this->db->getRow("SELECT ctype,cid,type FROM " . $GLOBALS['ecs']->table('nav') . " WHERE id = '$id' LIMIT 1");
...
//-- 切换是否显示
        ...
        if ($_REQUEST['act'] == 'toggle_ifshow') {
            ...
            $row = $this->db->getRow("SELECT type,ctype,cid FROM " . $GLOBALS['ecs']->table('nav') . " WHERE id = '$id' LIMIT 1");
