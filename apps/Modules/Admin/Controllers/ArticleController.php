...
//-- 文章列表
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['article_add'], 'href' => 'article.php?act=add'));
            ...
            $this->smarty->display('article_list.htm');
        }
...
//-- 翻页，排序
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($this->smarty->fetch('article_list.htm'), '',
...
//-- 添加文章
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['03_article_list'], 'href' => 'article.php?act=list'));
            $this->smarty->assign('form_action', 'insert');


            $this->smarty->display('article_info.htm');
        }
...
//-- 添加文章
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            $sql = "INSERT INTO " . $this->ecs->table('article') . "(title, cat_id, article_type, is_open, author, " .
...
//-- 编辑
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            $this->smarty->assign('article', $article);
            $this->smarty->assign('cat_select', article_cat_list(0, $article['cat_id']));
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['article_edit']);
            $this->smarty->assign('action_link', array('text' => $GLOBALS['_LANG']['03_article_list'], 'href' => 'article.php?act=list&' . list_link_postfix()));
...
//-- 删除文章主题
        ...
        if ($_REQUEST['act'] == 'remove') {
            ...
            if ($exc->drop($id)) {
                $this->db->query("DELETE FROM " . $this->ecs->table('comment') . " WHERE " . "comment_type = 1 AND id_value = $id");
...
//-- 将商品加入关联
        ...
        if ($_REQUEST['act'] == 'add_link_goods') {
            ...
            if ($article_id == 0) {
                $article_id = $this->db->getOne('SELECT MAX(article_id)+1 AS article_id FROM ' . $this->ecs->table('article'));
            }

            foreach ($add_ids as $key => $val) {
                $sql = 'INSERT INTO ' . $this->ecs->table('goods_article') . ' (goods_id, article_id) ' .
