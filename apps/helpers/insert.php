...
function insert_history()
{
    ...
    if (!empty(cookie('ectouch_history'))) {
        $where = db_create_in(cookie('ectouch_history'), 'goods_id');
...
function insert_member_info()
{
    ...
    if (session('user_id') > 0) {
        ...
        if (!empty(cookie('ectouch_username'))) {
            $GLOBALS['smarty']->assign('ecs_username', stripslashes(cookie('ectouch_username')));
        }
